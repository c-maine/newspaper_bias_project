...
def get_data_from_EventRegistry(countries_rest_dict):
    ...
    resp=q.execQuery(er, sortBy = "date", maxItems = 500,returnInfo=ReturnInfo(
...
def write_newspaper_to_db(newspaper_df, engine):
...
def write_main_to_db(main_df,engine):
...
def write_categories_to_db(categories_df, engine):
...
write_categories_to_db(category_df,db)
write_dates_to_db(main_df,db)
write_newspaper_to_db(newspaper_df,db)
write_main_to_db(main_df,db)
